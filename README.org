...
* Single-use Accounts, Smart Contracts and Invoicing
  ...
  especially if any of the governments are hostile to you or any of the other individuals involved
  in the transaction.  Worse yet, your business or family can be arbitrarily ejected from using the
  financial system, by an of the many intermediary parties involved in any traditional financial
  transaction.
...
*** Withdrawal Fees and Smart Contracts
   ...
   Usually, no Smart Contract invocation can be supported for the client's payment.  If you build a
   system that requires the client to have their own wallets, and to understand eg. Ethereum "Gas"
   pricing, you will prevent most clients from paying you.


** Single Use Accounts

   The solution to this problem is to use plain accounts that *nobody* has access to -- including
   your, or any payee!
   ...
   *can* create the exact same transaction performing the same actions -- and then *provide a random
   ...
   This is *not possible* using Bitcoin, due to its lack of general-purpose smart contracts, and the
   fact that one cannot "sign" transactions in advance to generate the "source" Account address: the
   transaction must contain specific information about the source UTXO (Unspent TransaXion Outputs)
   being spent, which is of course unavailable in advance.
