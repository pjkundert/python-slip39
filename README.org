...
* Single-use Accounts, Smart Contracts and Invoicing
  ...
  especially if any of the involved corporations or governments are hostile to you or any of the
  other individuals involved in the transaction.

  Worse yet, your business or family can be arbitrarily ejected from using the financial system, by
  an of the many intermediary parties involved in any traditional financial transaction.
...
** Using Plain HD Wallet Account
   ...
   "withdraw" the correct amount of Cryptocurrency to the given account.

*** Withdrawal Fees and Smart Contracts

    Since most exchanges do not allow withdrawal to an account that requires excessive fees; for
    example, to an Ethereum Smart Contract address, which may trigger functionality requiring excess
    "Gas" beyond what a normal transfer would require.

    This limits the amount of automation that can be attached to a "client" cryptocurrency payment
    address.  Usually, no Smart Contract invocation can be supported for the client's payment.  If
    you build a system that requires the client to have their own wallets, and to understand
    eg. Ethereum "Gas" pricing, you will prevent most clients from paying you.

** Some Drawbacks of using Plain Addresses for Client Payments
   
   So, generating a sequence of plain client addresses from an =xpub...= is a worthwhile solution to
   consider.

*** Fee Distribution

    Collecting up all the clients' payments later is a manual process, and if you wish to distribute
...
*** Distribution Failures
    ...
    this software uses your licensed module?  If the organization fails, or the software is
    abandoned, then clients who want to renew their license (or later find the software and install
    it) will probably not be able to pay the organization for it -- and your license fees also will
    go unpaid.

    Surely, there must be a way to deploy a sequence of interconnected Smart Contracts that can ensure
    that:

    - Any new client can uniquely allocate (and store some data with) a "Forwarder" address and pay for the product, 
    - Payment into that account address (and validation of its unique data) constitutes proof of licensing,
    - Payees (direct and indirect) can flush any such payments through to themselves (and others)

    These goals *can* be achieved using "Forwarder" Accounts

** Single Use "Forwarder" Accounts

   The solution to this problem is to use plain accounts that *nobody* has "private key" access to
   -- including you (the software issuer), or any client.

   These accounts are accessed by an Ethereum Smart Contract (the account address is actually the
   address of the contract).

   The *only* function of this "Forwarder" contract is collect the account's Ethereum and ERC-20
   token contents into the product's "Fee Distribution Contract" account.

*** Client "Forwarder" Contract

    Once the product's =MultiPayoutERC20= contract address is identified, the act of obtaining a
    unique client payee address is 
...
*** Client "Forwarder" Account Creation and Licensing   
    ...
    =MultiPayoutERC20= (product) Smart Contracts.
